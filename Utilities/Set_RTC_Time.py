...
class MainWindow(ttk.Frame):
    ...
    def current_time_frame(self):
        ...
        self.currentFrame = ttk.Frame(
            self.mastercontent, borderwidth=1, relief="solid", padding=(5)
        )
        ...
        # Create header
        self.currHeaderLabel = Label(
            self.currentFrame, text="Current Date / Time", justify="center"
        )
        ...
        self.currDay = Label(
            self.currentFrame, textvariable=self.currentDay, justify="center"
        )
        self.currDay.grid(row=2, column=1, sticky=(E, W))

        # Create day of week display
        self.currDayOWeekLabel = Label(
            self.currentFrame, text="Day of Week", justify="center"
        )
        self.currDayOWeekLabel.grid(row=1, column=2, sticky=(E, W))
        self.currDayOWeek = Label(
            self.currentFrame, textvariable=self.currentDayofWeek, justify="center"
        )
        ...
        self.currYear = Label(
            self.currentFrame, textvariable=self.currentYear, justify="center"
        )
        ...
        self.currHour = Label(
            self.currentFrame, textvariable=self.currentHour, justify="center"
        )
        self.currHour.grid(row=2, column=4, sticky=(E, W))

        # Create minutes display
        self.currMinuteLabel = Label(
            self.currentFrame, text="Minutes", justify="center"
        )
        self.currMinuteLabel.grid(row=1, column=5, sticky=(E, W))
        self.currMinute = Label(
            self.currentFrame, textvariable=self.currentMinute, justify="center"
        )
        self.currMinute.grid(row=2, column=5, sticky=(E, W))

        # Create seconds display
        self.currSecondLabel = Label(
            self.currentFrame, text="Seconds", justify="center"
        )
        self.currSecondLabel.grid(row=1, column=6, sticky=(E, W))
        self.currSecond = Label(
            self.currentFrame, textvariable=self.currentSecond, justify="center"
        )
    ...
    def new_time_frame(self):
        ...
        self.setHeaderLabel = Label(
            self.setFrame, text="Set Date / Time", justify="center"
        )
        self.setHeaderLabel.grid(row=0, column=0, columnspan=7)

        # Create month input
        self.setMonthLabel = Label(self.setFrame, text="Month", justify="center")
        self.setMonthLabel.grid(row=1, column=0, sticky=(E, W))
        self.monthEntry = Entry(self.setFrame, textvariable=self.newMonth)
        ...
        self.dayEntry = Entry(self.setFrame, textvariable=self.newDay)
        self.dayEntry.grid(row=2, column=1, sticky=(E, W))

        # Create day of week input
        self.setDayOWeekLabel = Label(
            self.setFrame, text="Day of Week", justify="center"
        )
        self.setDayOWeekLabel.grid(row=1, column=2, sticky=(E, W))
        self.dayOWeekEntry = Entry(self.setFrame, textvariable=self.newDayofWeek)
        self.dayOWeekEntry.grid(row=2, column=2, sticky=(E, W))

        # Create year input
        self.setYearLabel = Label(self.setFrame, text="Year", justify="center")
        self.setYearLabel.grid(row=1, column=3, sticky=(E, W))
        self.yearEntry = Entry(self.setFrame, textvariable=self.newYear)
        self.yearEntry.grid(row=2, column=3, sticky=(E, W))

        # Create hour input
        self.setHourLabel = Label(self.setFrame, text="Hour", justify="center")
        self.setHourLabel.grid(row=1, column=4, sticky=(E, W))
        self.hourEntry = Entry(self.setFrame, textvariable=self.newHour)
        self.hourEntry.grid(row=2, column=4, sticky=(E, W))

        # Create minute input
        self.setMinuteLabel = Label(self.setFrame, text="Minute", justify="center")
        self.setMinuteLabel.grid(row=1, column=5, sticky=(E, W))
        self.minuteEntry = Entry(self.setFrame, textvariable=self.newMinute)
        self.minuteEntry.grid(row=2, column=5, sticky=(E, W))

        # Create second input
        self.setSecondLabel = Label(self.setFrame, text="Second", justify="center")
        self.setSecondLabel.grid(row=1, column=6, sticky=(E, W))
        self.secondEntry = Entry(self.setFrame, textvariable=self.newSecond)
        self.secondEntry.grid(row=2, column=6, sticky=(E, W))

        # Create set button
        self.setButton = Button(
            self.setFrame, text="Set Time", command=self.set_time_button_press
        )
    ...
    def set_time_button_press(self):
        ...
        new_time = [
            int(self.newHour.get()),
            int(self.newMinute.get()),
            int(self.newSecond.get()),
        ]
        new_date = [
            int(self.newDayofWeek.get()),
            int(self.newDay.get()),
