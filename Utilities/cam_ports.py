...
def find_cams(port: int):
    ...
    # Pi 5
    port5 = [0, 1, 3, 2][port]
    file = f"/sys/devices/platform/axi/1000120000.pcie/1f00{port % 2 + 2}00000.usb/xhci-hcd.{port5 % 2}/usb{port5 % 2 * 2 + 1}/{port5 % 2 * 2 + 1}-{port5 // 2 + 1}/{port5 % 2 * 2 + 1}-{port5 // 2 + 1}:1.0/video4linux"
    if os.path.exists(file):
        files = [
            int(x[5:])
            for x in os.listdir(file)
