...
def round_tuple(tup, p, n=1):
    return (
        "("
        + ", ".join(
            (("{:" + str(p + n + 2) + "." + str(p) + "f}").format(v) for v in tup)
        )
        + ")"
    )
...
def main():
    ...
    with open(logFilename, "w") as log_file:
        ...
        while not stop:
            ...
            if useNavx:
                gyroInit = (
                    navxTable.getNumber("ZeroGyro", 0) if networkTablesConnected else 0
                )  # Check for signal to re-zero gyro
                if gyroInit == 1:
                    navx.reset_gyro()
                ...
                if navxLoopCount + 1 == navxTesting:
                    print(
                        "angle: {:7.2f}\torientation: {}\tacceleration: {}\tvelocity: {}\tposition: {}".format(
                            gyroAngle,
                            round_tuple(navx.read_orientation(), 2, 3),
                            round_tuple(navx.read_acceleration(), 4),
                            round_tuple(navx.read_velocity(), 4),
                            round_tuple(navx.read_position(), 4),
                        )
                    )
                    navxLoopCount = 0
            else:
