...
class BallOnlyVisionLibrary(VisionBase):
    ...
    def find_objects(
        ...
        ballContours = self.process_image_contours(
            imgRaw, ballHSVMin, ballHSVMax, False
        )
        ...
        if len(ballContours) > 0:
            ...
            for contour in sortedContours:
                ...
                if radius > float(minRadius):
                    ...
                    angleToBall = math.degrees(
                        math.atan((offsetInInches / distanceToTargetPlane))
                    )
                    distanceToBall = (
                        math.cos(math.radians(angleToBall)) * distanceToTargetPlane
                    )
                    screenPercent = (
                        math.pi * radius * radius / (cameraWidth * cameraHeight)
                    )
...
class BallVisionLibrary(VisionBase):
    ...
    def find_objects(
        ...
        # Find contours in the mask and clean up the return style from OpenCV
        ballContours = self.process_image_contours(
            imgRaw, ballHSVMin, ballHSVMax, False, True
        )
        ...
        if len(ballContours) > 0:
            ...
            for contour in sortedContours:
                ...
                if (aspectRatio >= 1 - ballTolerance) and (
                    aspectRatio <= 1 + ballTolerance
                ):
                    # Get ball radius values
                    minRadius = config["MINRADIUS"]
                    ballRadius = config["RADIUS"]
                    ...
                    if radius > float(minRadius):  # in pixel units
                        ...
                        inches_per_pixel = (
                            float(ballRadius) / radius
                        )  # set up a general conversion factor [ballRadius{in inches} / radius{in pixels}]
                        distanceToTargetPlane = inches_per_pixel * (
                            cameraWidth / (2 * math.tan(math.radians(cameraFOV)))
                        )  # distanceToBall = targetPlaneSize/(2*tangent of half of viewing angle
                        offsetInInches = inches_per_pixel * (x - cameraWidth / 2)
                        angleToBall = math.degrees(
                            math.atan((offsetInInches / distanceToTargetPlane))
                        )
